...
def get_favourite_product(dataframe_dict, gender):
    favourite_product, favourite_products = get_favourite_product_by_gender(dataframe_dict, gender)

    favourite_products = favourite_products.index.tolist()
    number_of_times_purchased = favourite_product.max()

    print(f"The favourite products for {gender.lower()}s have product_id's: ", favourite_products,
